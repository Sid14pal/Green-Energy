...
$.extend($.easing, {
	...
	function() {
		var e, t, i, n;
		Riode.$window = $(window), Riode.$body = $(document.body), Riode.status = "", Riode.minDesktopWidth = 992, Riode.isIE = navigator.userAgent.indexOf("Trident") >= 0, Riode.isEdge = navigator.userAgent.indexOf("Edge") >= 0, Riode.isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent), Riode.defaults = {
			animation: {
				name: "fadeIn",
				duration: "1.2s",
				delay: ".2s"
			...
			slider: {
				responsiveClass: !0,
				navText: ['<span data-feather="arrow-right"></span>', '<span data-feather="arrow-left"></span>'],
				checkVisible: !1,
				items: 1,
			...
			sliderThumbs: {
				...
				dots: !1,
				nav: !0,
				navText: ['<span data-feather="arrow-right"></span>', '<span data-feather="arrow-right"></span>']
			},
			stickyContent: {
			...
			var n = window.pageXOffset,
				...
				d = void 0 === t ? 0 : t,
				l = void 0 === i ? 0 : i;
			return r + a.height + l >= o && r <= o + window.innerHeight + l && s + a.width + d >= n && s <= n + window.innerWidth + d
			...
			var i = 0,
				n = void 0 === t ? 600 : t;
			i = "number" == typeof e ? e : Riode.$(e).offset().top, $("html,body").stop().animate({
				scrollTop: i
			}, n)
			...
			return s.val = n((function r(d) {
				o || (o = a = d), !i || d - o < i ? d - a > t ? (s.val = n(r), e(), a = d) : s.val = n(r) : e()
			})), s
			...
			return e.prototype.init = function(e) {
					...
					a.$wrapper = e, a.isQuickview = !!e.closest(".mfp-content").length, a._isPgvertical = !1, a.isQuickview && (o = o.bind(this), Riode.ratingTooltip()), s.on("initialized.owl.carousel", (function(e) {
							document.body.classList.contains("home") || a.isQuickview || s.append(''), s.parent().hasClass("product-gallery-degree") && (a.isQuickView || s.append('<a href="#" class="product-gallery-btn product-degree-viewer" title="Product 360 Degree Gallery"><i class="w-icon-rotate-3d"></i></a>')),
								function(e) {
						...
						function(e) {
							e.$wrapper.on("click", ".btn-compare", (function(t) {
								...
								var i = e.$wrapper,
									n = i.find(".product-name").text();
								(i.closest(".product-popup").length || document.body.classList.contains("home")) & "disabled" != i.find(".btn-product.btn-cart").attr("disabled") && Riode.Minipopup.open({
									message: "Successfully Added",
			...
			function t(e) {
				var t = $(e.currentTarget).closest(".product-single");
				"disabled" != t.find(".btn-product.btn-cart").attr("disabled") && Riode.Minipopup.open({
					message: "Successfully Added",
					productClass: "product-compare",
					name: t.find("h1.product-name").text(),
			...
			return e.zoomImage = function() {
				...
				}, Riode.defaults.sliderPresets["product-single-carousel"].onInitialized = Riode.defaults.sliderPresets["product-gallery-carousel"].onInitialized = e.zoomImage, Riode.defaults.sliderPresets["product-single-carousel"].onRefreshed = Riode.defaults.sliderPresets["product-gallery-carousel"].onRefreshed = e.zoomImageRefresh, e.prototype.init = function(e, d) {
					...
					l.forEach((function(e) {
						...
						this.loop = !1
					})), $.extend(!0, c, Riode.parseOptions(e.attr("data-owl-options")), d), o = o.bind(this), s = s.bind(this), r = r.bind(this), e.on("initialize.owl.carousel", t).on("initialized.owl.carousel", i).on("translated.owl.carousel", n), e.hasClass("animation-slider") && e.on("initialized.owl.carousel", o).on("resized.owl.carousel", a).on("translate.owl.carousel", s).on("translated.owl.carousel", r), e.owlCarousel(c)
				},
				function(t, i) {
			...
			return {
				...
				open: function(n, r) {
					...
					c.isPurchased ? c.detailTemplate = Riode.parseTemplate(c.purchasedTemplate, c) : c.detailTemplate = Riode.parseTemplate(null != c.count ? c.priceQuantityTemplate : c.priceTemplate, c), null != c.rating && (c.detailTemplate += Riode.parseTemplate(c.ratingTemplate, c)), d = $(Riode.parseTemplate(c.template, c)), l.space = c.space, d.appendTo(e).css("top", -t).find("img")[0].onload = function() {
						t += d[0].offsetHeight + l.space, d.addClass("show"), d.offset().top - window.pageYOffset < 0 && (l.close(), d.css("top", -t + d[0].offsetHeight + l.space)), d.on("mouseenter", (function() {
							l.pause()
							...
							l.resume()
						})), i.push(d), o.push(c.delay), o.length > 1 || (a = setInterval(s, 200)), r && r(d)
					}
				},
				close: function(e) {
					var n = void 0 === e ? 0 : e,
			...
			initWishlistButton: function(e) {
				Riode.$body.on("click", e, (function(e) {
					...
					t.toggleClass("added").addClass("load-more-overlay loading"), setTimeout((function() {
						t.removeClass("load-more-overlay loading").find("i").toggleClass("d-icon-heart").toggleClass(""), t.hasClass("added") ? t.attr("title", "Remove from wishlist") : t.attr("title", "Add to wishlist")
	...
	fetch("header.html")
					...
					if (keyword.length > 0) {
						...
						if (filtered.length > 0) {
							filtered.forEach(p => {
								...
								item.innerHTML = `
          <a href="product-details.html?id=${p.id}"><img src="${p.images[0]}" alt="${p.title}"></a>
          <div>
            <a href="product-details.html?id=${p.id}"><p class="name">${p.title}</p></a>
            <a href="product-details.html?id=${p.id}"><p class="price">$${p.price}</p></a>
          </div>
        `;
				...
				const cartTableBody = document.querySelector("#dropdown-box");
     ...
     <div class="product product-cart">
                        <figure class="product-media">
                           ...
                           <button class="btn btn-link btn-close product-remove" data-index="${index}">
                           X
                           </button>
                        </figure>
                        <div class="product-detail">
                           <a href="demo-food2-product.html" class="product-name">${item.title}</a>
                           <div class="price-box">
			...
			if (window.feather) {
				...
				form.addEventListener("submit", function(e) {
					...
					if (email === "" || !regex.test(email)) {
						...
						// ✅ Email is valid → here you can send data to server
